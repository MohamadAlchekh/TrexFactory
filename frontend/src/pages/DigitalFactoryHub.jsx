...
const DigitalFactoryHub = () => {
  ...
  return (
    ...
    >
      <div className="mb-6">
        <h2 className="text-2xl font-bold font-sans text-gray-900 dark:text-white uppercase tracking-tight">Executive Summary</h2>
        <p className="text-sm font-mono text-gray-500 dark:text-dark-muted">{executiveSummary?.period ?? 'Tarihsel MES Log Verileri'}</p>
      </div>

      {/* TOP ROW: IMPACT KPIs — backend verilerine bağlı */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <motion.div variants={itemVariants} className="bg-white dark:bg-dark-surface border border-red-200 dark:border-red-900/30 p-5 rounded-none shadow-sm relative overflow-hidden group">
          <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
            <TrendingDown className="w-16 h-16 text-red-500" />
          </div>
          <h3 className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Toplam Aktif Alarm</h3>
          <div className="flex items-end gap-2">
            <span className="text-4xl font-black font-mono text-red-600 dark:text-red-500">{totalAlerts}</span>
            <span className="text-sm text-gray-400 mb-1 font-bold">Adet</span>
          </div>
          <div className="mt-4 pt-4 border-t border-gray-100 dark:border-dark-border text-xs text-red-500 dark:text-red-400 flex items-center gap-1 font-medium">
            <AlertTriangle className="w-4 h-4" /> Plansız duruş alarm kayıtları
          </div>
        </motion.div>

        <motion.div variants={itemVariants} className="bg-white dark:bg-dark-surface border border-orange-200 dark:border-orange-900/30 p-5 rounded-none shadow-sm relative overflow-hidden group">
          <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
            <Activity className="w-16 h-16 text-orange-500" />
          </div>
          <h3 className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Ortalama OEE</h3>
          <div className="flex items-end gap-2">
            <span className="text-4xl font-black font-mono text-orange-600 dark:text-orange-500">%{avgOee}</span>
          </div>
          <div className="mt-4 pt-4 border-t border-gray-100 dark:border-dark-border text-xs text-orange-500 dark:text-orange-400 flex items-center gap-1 font-medium">
            <Zap className="w-4 h-4" /> {totalMachines} makinenin ağırlıklı ortalaması
          </div>
        </motion.div>

        <motion.div variants={itemVariants} className="bg-white dark:bg-dark-surface border border-green-200 dark:border-green-900/30 p-5 rounded-none shadow-sm relative overflow-hidden group">
          <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
            <DollarSign className="w-16 h-16 text-green-500" />
          </div>
          <h3 className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Toplam İzlenen Makine</h3>
          <div className="flex items-end gap-2">
            <span className="text-4xl font-black font-mono text-green-600 dark:text-green-500">{totalMachines}</span>
            <span className="text-sm text-gray-400 mb-1 font-bold">Ünite</span>
          </div>
          <div className="mt-4 pt-4 border-t border-gray-100 dark:border-dark-border text-xs text-green-600 dark:text-green-400 flex items-center justify-between font-bold">
          ...
          <div className="flex flex-col lg:flex-row gap-6 items-start lg:items-center justify-between">
            <div className="flex-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="bg-white/50 dark:bg-dark-bg/50 p-3 border border-red-100 dark:border-red-900/20">
                  <div className="text-[10px] uppercase text-gray-500 font-bold mb-1">Toplam Alarm</div>
                  <div className="font-mono font-bold text-red-600 dark:text-red-400">{totalAlerts} Kayıt</div>
                </div>
                <div className="bg-white/50 dark:bg-dark-bg/50 p-3 border border-orange-100 dark:border-orange-900/20">
                  <div className="text-[10px] uppercase text-gray-500 font-bold mb-1">En Sık Hata</div>
                  <div className="font-mono font-bold text-orange-600 dark:text-orange-400 text-xs">{topStoppage}</div>
                </div>
                <div className="bg-white/50 dark:bg-dark-bg/50 p-3 border border-green-100 dark:border-green-900/20">
                  <div className="text-[10px] uppercase text-gray-500 font-bold mb-1">OEE Ortalaması</div>
                  <div className="font-mono font-bold text-green-600 dark:text-green-400">%{avgOee}</div>
                </div>
              </div>
            </div>
            
            <div className="shrink-0 w-full lg:w-auto bg-dark-bg dark:bg-black/40 p-4 border border-gray-200 dark:border-gray-800">
              <div className="text-xs uppercase text-gray-400 font-bold mb-3">Sistem Önerisi</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <motion.div variants={itemVariants} className="bg-white dark:bg-dark-surface border border-gray-200 dark:border-dark-border p-5 rounded-none shadow-sm">
          <h3 className="text-sm font-bold text-gray-800 dark:text-gray-200 uppercase mb-4">Aylar Bazında Alarm Sıklığı (Zaman Çizelgesi)</h3>
        ...
        <motion.div variants={itemVariants} className="bg-white dark:bg-dark-surface border border-gray-200 dark:border-dark-border p-5 rounded-none shadow-sm">
          <h3 className="text-sm font-bold text-gray-800 dark:text-gray-200 uppercase mb-4">Alarm Türleri (Pareto — Gerçek Backend Verisi)</h3>
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
