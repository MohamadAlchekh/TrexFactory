  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVariants} className="flex flex-col xl:flex-row justify-between items-start gap-6 mb-8">
        <div>
          <h1 className="text-3xl font-bold text-white uppercase tracking-tight">Finansal Senaryo Raporu</h1>
          <p className="text-sm font-mono text-white mt-1">CAPEX / OPEX Yatırım Getirisi ve Başabaş (Breakeven) Analizi</p>
        </div>
        
        <div className="flex-1 max-w-3xl bg-[#1a2235] border border-gray-600 dark:border-gray-500 p-5 flex flex-col md:flex-row items-start md:items-center justify-between shadow-[0_10px_30px_rgba(0,0,0,0.5)] relative overflow-hidden">
          ...
          <div className="flex items-start gap-4 z-10 w-full">
            ...
            <div className="flex-1">
              <div className="flex justify-between items-center border-b border-gray-700 pb-2 mb-3">
                ...
                <span className="text-[10px] font-mono text-white">REF: TR-INV-{Date.now().toString().slice(-4)}</span>
              </div>
      ...
      {/* SCENARIO SELECTORS */}
      <motion.div variants={itemVariants} className="bg-[#1a2235] p-4 border border-white/10 mb-6 shadow-sm">
        <h3 className="text-[10px] font-bold text-white uppercase tracking-widest mb-3">Senaryo Seçimi</h3>
              ...
              <button 
                ...
                className={`flex items-center gap-2 px-3 py-2 border text-sm font-bold transition-all ${
                  isSelected 
                    ...
                    : 'border-white/10 dark:border-gray-800 text-white hover:border-gray-300 dark:hover:border-gray-600'
                }`}
              >
      ...
      {/* DATA TABLE */}
      <motion.div variants={itemVariants} className="bg-[#1a2235] border border-white/10 mb-6 shadow-sm overflow-x-auto">
        <table className="w-full text-sm text-left border-collapse">
          <thead className="bg-[#1a2235] text-[10px] uppercase font-bold text-white tracking-wider">
            <tr>
              <th className="px-4 py-3 border-b border-white/10">Senaryo</th>
              <th className="px-4 py-3 border-b border-white/10 text-right">A (%)</th>
              <th className="px-4 py-3 border-b border-white/10 text-right">P (%)</th>
              ...
              <th className="px-4 py-3 border-b border-white/10 text-right bg-blue-50/50 dark:bg-blue-900/10">OEE (%)</th>
              <th className="px-4 py-3 border-b border-white/10 text-right">Delta OEE</th>
              <th className="px-4 py-3 border-b border-white/10 text-right">Brüt Fayda ($)</th>
              <th className="px-4 py-3 border-b border-white/10 text-right">Amortisman</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-200 dark:divide-dark-border">
            {selectedData.map((d) => (
              <tr key={d.id} className={`${d.id === 'S0' ? 'bg-white/5 dark:bg-gray-900/50' : 'hover:bg-[#1a2235] dark:hover:bg-gray-800/30'}`}>
                ...
                <td className={`px-4 py-3 text-right font-mono ${d.a === maxValues.a && d.id !== 'S0' ? 'text-green-500 font-black' : 'text-white'}`}>{d.a.toFixed(1)}</td>
                <td className={`px-4 py-3 text-right font-mono ${d.p === maxValues.p && d.id !== 'S0' ? 'text-green-500 font-black' : 'text-white'}`}>{d.p.toFixed(1)}</td>
                <td className={`px-4 py-3 text-right font-mono ${d.q === maxValues.q && d.id !== 'S0' ? 'text-green-500 font-black' : 'text-white'}`}>{d.q.toFixed(1)}</td>
                <td className={`px-4 py-3 text-right font-mono bg-blue-50/50 dark:bg-blue-900/10 ${d.oee === maxValues.oee && d.id !== 'S0' ? 'text-green-500 font-black text-base' : 'text-white font-bold'}`}>{d.oee.toFixed(1)}</td>
                <td className={`px-4 py-3 text-right font-mono ${d.deltaOee === maxValues.deltaOee && d.id !== 'S0' ? 'text-green-500 font-black' : 'text-white'}`}>+{d.deltaOee.toFixed(1)}</td>
                <td className={`px-4 py-3 text-right font-mono ${d.brutFayda === maxValues.brutFayda && d.id !== 'S0' ? 'text-green-500 font-black' : 'text-white'}`}>${new Intl.NumberFormat('en-US').format(d.brutFayda)}</td>
                <td className={`px-4 py-3 text-right font-mono ${d.amortisman === minAmortisman && d.id !== 'S0' ? 'text-green-500 font-black' : 'text-white'}`}>{d.id === 'S0' ? '-' : `${d.amortisman} Gün`}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <motion.div variants={itemVariants} className="bg-[#1a2235] p-6 border border-white/10 shadow-sm flex flex-col h-[350px]">
        ...
        <motion.div variants={itemVariants} className="bg-[#1a2235] p-6 border border-white/10 shadow-sm flex flex-col h-[350px]">
          ...
          <p className="text-[10px] text-white uppercase font-bold mb-2 tracking-widest">Zaman: 720 Gün (24 Ay)</p>
          <div className="flex-1 w-full h-full">
            <ResponsiveContainer width="100%" height="100%">
