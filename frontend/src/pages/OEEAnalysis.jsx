    ...
    <div className="flex flex-col items-center justify-center relative" style={{ width: size, height: size }}>
      ...
      <div className="absolute flex flex-col items-center justify-center inset-0">
        <span className={`${isLarge ? 'text-4xl' : 'text-2xl'} font-black font-mono text-white`}>
          {Math.round(value)}<span className="text-sm text-white">%</span>
        </span>
        <span className="text-xs font-bold text-white uppercase tracking-widest mt-1">{label}</span>
      </div>
    </div>
  );
};
...
const CustomTooltip = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-[#1a2235] border border-[#30363d] p-3 rounded-none z-50 relative">
...
const OEEAnalysis = () => {
  ...
  return (
    ...
    >
      <div className="mb-8">
        <h2 className="text-2xl font-bold font-sans text-white uppercase tracking-tight">OEE Baseline Dashboard</h2>
        <p className="text-sm font-mono text-white">Genel Ekipman Verimliliği Analizi</p>
      </div>

      {/* Machine Selector */}
      <motion.div variants={itemVariants} className="mb-10 bg-[#1a2235] p-5 border border-white/10 rounded-none shadow-sm flex flex-col md:flex-row md:items-center gap-6">
        <div className="flex-1">
          <label className="block text-xs font-bold text-white uppercase tracking-wider mb-2">Makine / Hat Seçimi</label>
          <select 
            value={selectedMachineId}
            onChange={(e) => setSelectedMachineId(e.target.value)}
            className="w-full md:w-64 bg-[#1a2235] border border-gray-300 dark:border-gray-700 text-white text-sm rounded-none focus:ring-blue-500 focus:border-blue-500 block p-2.5 outline-none font-medium"
          >
            {machineList.map(m => (
        ...
        <div className="flex flex-col border-t md:border-t-0 md:border-l border-white/10 pt-4 md:pt-0 md:pl-6">
          <span className="text-[10px] text-white uppercase font-bold tracking-widest mb-1">Seçili Birim UID</span>
          <span className="font-mono text-sm text-white dark:text-gray-300 bg-white/5 bg-[#1a2235] px-3 py-1 border border-white/10 dark:border-gray-800">
            {selectedMachine.uid}
          </span>
        </div>
      </motion.div>
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-center">
        ...
        {/* OEE Gauge (Largest) */}
        <div className="bg-[#1a2235] border border-blue-200 dark:border-blue-900/30 p-8 rounded-none shadow-sm flex flex-col items-center justify-center col-span-1 h-full min-h-[300px]">
          <h3 className="text-sm font-bold text-white uppercase tracking-wider mb-6 text-center">Birleşik OEE</h3>
          <CircularProgress 
        ...
        <div className="col-span-1 lg:col-span-3 grid grid-cols-1 md:grid-cols-3 gap-6 h-full">
          ...
          {/* Availability */}
          <div className="bg-[#1a2235] border border-white/10 p-6 rounded-none shadow-sm flex flex-col items-center justify-center">
            <h3 className="text-sm font-bold text-white uppercase tracking-wider mb-4 text-center">Availability (A)</h3>
            <CircularProgress 
          ...
          {/* Performance */}
          <div className="bg-[#1a2235] border border-white/10 p-6 rounded-none shadow-sm flex flex-col items-center justify-center">
            <h3 className="text-sm font-bold text-white uppercase tracking-wider mb-4 text-center">Performance (P)</h3>
            <CircularProgress 
          ...
          {/* Quality */}
          <div className="bg-[#1a2235] border border-green-200 dark:border-green-900/30 p-6 rounded-none shadow-sm flex flex-col items-center justify-center relative">
            <h3 className="text-sm font-bold text-white uppercase tracking-wider mb-4 text-center flex flex-col items-center">
              Quality (Q)
              <span className="text-[9px] text-white mt-1 lowercase tracking-normal">(Simüle Edilmiş Sabit Değer)</span>
            </h3>
            <CircularProgress 
      ...
      <motion.div variants={itemVariants} className="mt-6 grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 bg-[#1a2235] border border-white/10 p-5 rounded-none shadow-sm flex flex-col">
          <h3 className="text-sm font-bold text-white uppercase mb-4">OEE, A ve P Trendi (Ağu 2025 - May 2026)</h3>
        ...
        <div className="xl:col-span-1 bg-[#1a2235] border border-white/10 p-0 rounded-none shadow-sm flex flex-col overflow-hidden">
          <div className="p-4 border-b border-white/10 bg-[#1a2235]">
            <h3 className="text-sm font-bold text-white uppercase">En Kötü Günler</h3>
            <p className="text-[10px] text-white mt-1 uppercase tracking-wider">Plansız Duruşa Göre Sıralı</p>
          </div>
          <div className="flex-1 overflow-auto custom-scrollbar">
            <table className="w-full text-left text-sm">
              <thead className="bg-[#1a2235] text-white text-xs uppercase sticky top-0">
                <tr>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-dark-border">
                {worstDaysData.map((row, idx) => (
                  <tr key={idx} className="hover:bg-blue-50 dark:hover:bg-blue-900/20 cursor-pointer transition-colors group">
                    <td className="px-4 py-2 font-mono text-white">{row.date}</td>
      ...
      <motion.div variants={itemVariants} className="mt-6 bg-[#1a2235] border border-white/10 p-5 rounded-none shadow-sm">
        <h3 className="text-sm font-bold text-white uppercase mb-4">Plansız Duruş Kategorileri (Saat)</h3>
