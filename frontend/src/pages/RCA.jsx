    ...
    >
      ...
      <div className="hidden print:block absolute inset-0 bg-white text-black p-8 z-50">
        ...
        <div className="mb-6">
          ...
          <table className="w-full text-sm border-collapse border border-black">
            <tbody>
              <tr>
                <td className="border border-black py-1 px-2 font-bold bg-gray-100 w-1/4">Makine / Hat</td>
                <td className="border border-black py-1 px-2 font-mono">{selectedMachine?.name || "Makine 1"}</td>
              </tr>
              <tr>
                <td className="border border-black py-1 px-2 font-bold bg-gray-100">Hata Mesajı</td>
        ...
        <div className="mb-6">
          ...
          <div className="border border-black p-4 font-mono text-sm leading-relaxed text-justify bg-gray-50">
      ...
      <div className="print:hidden">
        ...
        <motion.div variants={itemVariants} className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            <h2 className="text-2xl font-bold font-sans text-gray-900 dark:text-white uppercase tracking-tight">Platin RCA Zaman Çizelgesi</h2>
            <p className="text-sm font-mono text-gray-500 dark:text-dark-muted">Kök Neden ve Kaskad Analizi</p>
          </div>

          <div className="flex gap-2">
            <button onClick={handleExportPDF} className="flex items-center gap-2 px-3 py-1.5 bg-white dark:bg-dark-surface border border-gray-200 dark:border-dark-border rounded-none text-sm font-bold text-gray-700 dark:text-white hover:bg-gray-50 dark:hover:bg-dark-bg transition-colors">
              <Printer className="w-4 h-4 text-gray-500" /> Yazdır
            </button>
            <button onClick={handleDownloadPDF} className="flex items-center gap-2 px-3 py-1.5 bg-white dark:bg-dark-surface border border-gray-200 dark:border-dark-border rounded-none text-sm font-bold text-gray-700 dark:text-white hover:bg-gray-50 dark:hover:bg-dark-bg transition-colors">
        ...
        {/* SELECTOR MENU */}
        <motion.div variants={itemVariants} className="bg-white dark:bg-dark-surface p-4 border border-gray-200 dark:border-dark-border rounded-none shadow-sm mb-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
          <div className="space-y-1">
            <label className="text-[10px] uppercase font-bold text-gray-500 tracking-wider flex items-center gap-1"><Monitor className="w-3 h-3" /> Makine</label>
            <select
              value={selectedMachineId}
              onChange={e => setSelectedMachineId(e.target.value)}
              className="w-full bg-gray-50 dark:bg-dark-bg border border-gray-300 dark:border-gray-700 text-sm p-2 outline-none text-gray-900 dark:text-white font-medium rounded-none"
            >
              {machineList.map(m => (
          ...
          <div className="space-y-1">
            <label className="text-[10px] uppercase font-bold text-gray-500 tracking-wider flex items-center gap-1"><Calendar className="w-3 h-3" /> Başlangıç Tarihi</label>
            <input
              type="date"
              value={startDate}
              onChange={e => setStartDate(e.target.value)}
              className="w-full bg-gray-50 dark:bg-dark-bg border border-gray-300 dark:border-gray-700 text-sm p-2 outline-none text-gray-900 dark:text-white font-medium rounded-none focus:ring-1 focus:ring-blue-500"
            />
          </div>
          <div className="space-y-1">
            <label className="text-[10px] uppercase font-bold text-gray-500 tracking-wider flex items-center gap-1"><Calendar className="w-3 h-3" /> Bitiş Tarihi</label>
            <input
              type="date"
              value={endDate}
          ...
          <div className="space-y-1">
            <label className="text-[10px] uppercase font-bold text-gray-500 tracking-wider flex items-center gap-1"><Calendar className="w-3 h-3" /> Olay Tarihi</label>
            <select className="w-full bg-gray-50 dark:bg-dark-bg border border-gray-300 dark:border-gray-700 text-sm p-2 outline-none text-gray-900 dark:text-white font-medium rounded-none">
              <option>{startDate} 14:25:50</option>
            </select>
          </div>
          <div className="space-y-1">
            <label className="text-[10px] uppercase font-bold text-gray-500 tracking-wider flex items-center gap-1"><AlertOctagon className="w-3 h-3" /> Alarm/Olay</label>
            <select className="w-full bg-gray-50 dark:bg-dark-bg border border-red-300 dark:border-red-900/50 text-sm p-2 outline-none text-red-600 dark:text-red-400 font-bold rounded-none">
        ...
        <motion.div variants={itemVariants} className="bg-[#0d1117] border border-blue-500/30 p-5 mb-6 rounded-none relative overflow-hidden font-mono text-xs shadow-lg">
          ...
          <p className="text-gray-400 mb-4 font-sans leading-relaxed text-left">
          ...
          <div className="flex flex-col md:flex-row gap-4 items-stretch">
            <div className="flex-1 bg-black/80 border border-gray-800 p-4 h-36 overflow-y-auto custom-scrollbar flex flex-col gap-1 text-green-400 font-mono">
              {rebuildLogs.length === 0 ? (
                <span className="text-gray-600 text-left">&gt; Hazır. Başlatmak için sağdaki butona tıklayın...</span>
              ) : (
            ...
            <div className="md:w-64 shrink-0 flex flex-col justify-between border border-gray-800 bg-gray-900/50 p-4">
              <div className="space-y-2">
                <div className="text-[10px] text-gray-500 uppercase font-bold text-left">Aktif Zaman Dilimi</div>
          ...
          {isRebuilt && (
            ...
            >
              <div className="bg-black/40 p-2.5 border border-gray-900">
                <span className="text-[9px] text-gray-500 uppercase font-bold block mb-1">Çekilen stoppage_slice</span>
                <span className="text-sm font-bold text-yellow-500 font-mono">{shiftStats.stoppage} adet dilim</span>
              </div>
              <div className="bg-black/40 p-2.5 border border-gray-900">
                <span className="text-[9px] text-gray-500 uppercase font-bold block mb-1">Çekilen counter_slice</span>
                <span className="text-sm font-bold text-blue-500 font-mono">{shiftStats.counter} adet veri</span>
              </div>
              <div className="bg-black/40 p-2.5 border border-gray-900">
                <span className="text-[9px] text-gray-500 uppercase font-bold block mb-1">Hesaplanan OEE Ort.</span>
                <span className="text-sm font-bold text-green-500 font-mono">%{shiftStats.oee}</span>
              </div>
              <div className="bg-black/40 p-2.5 border border-gray-900">
                <span className="text-[9px] text-gray-500 uppercase font-bold block mb-1">Kayıp Zaman Oranı</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {/* LEFT: TIMELINE */}
          <motion.div variants={itemVariants} className="bg-white dark:bg-dark-surface p-6 border border-gray-200 dark:border-dark-border rounded-none shadow-sm h-full">
            ...
            <div className="relative border-l-2 border-gray-200 dark:border-gray-800 ml-4 space-y-8">
              {filteredTimelineEvents.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-gray-500 dark:text-dark-muted font-mono">
                  ...
                  <span className="text-[10px] text-gray-400 mt-1">Geçerli tarihler: 2026-01-12, 2026-01-21, 2026-02-25</span>
                </div>
              ) : (
                filteredTimelineEvents.map((event, idx) => (
                  <div key={event.id} className="relative pl-8">
                    {/* Icon Marker */}
                    <div className={`absolute -left-[17px] top-0.5 w-8 h-8 flex items-center justify-center bg-white dark:bg-dark-surface border-2 rounded-full z-10 ${event.type === 'alarm' ? 'border-red-500' : 'border-orange-500'
                      }`}>
                      {event.type === 'alarm' ? (
                    ...
                    {/* Content */}
                    <div>
                      <span className="font-mono text-xs font-bold text-gray-500 dark:text-gray-400">{event.time}</span>
          ...
          <motion.div variants={itemVariants} className="relative bg-white dark:bg-dark-surface p-6 border border-gray-200 dark:border-dark-border rounded-none shadow-sm h-full flex flex-col">
            {!hasData && (
              <div className="absolute inset-0 bg-white/95 dark:bg-[#0d1117]/95 flex flex-col items-center justify-center z-30 p-4">
                <AlertCircle className="w-10 h-10 text-red-500 mb-2 animate-pulse" />
                <span className="text-sm font-bold text-gray-900 dark:text-white font-mono uppercase">AKTİF TELEMETRİ KAYDI BULUNAMADI</span>
                <span className="text-xs text-gray-500 dark:text-dark-muted mt-1 font-mono text-center">Girilen tarihte makine aktif üretim yapmamıştır veya kayıt yoktur.<br />Lütfen 2026-01-12, 2026-01-21 veya 2026-02-25 tarihlerini seçin.</span>
              </div>
            )}
            ...
            <h3 className="text-sm font-bold text-gray-800 dark:text-gray-200 uppercase mb-2 flex items-center gap-2">
              <Activity className="w-4 h-4 text-blue-500" /> Sensör Trendi: PATH_LOAD %
            </h3>
            <p className="text-[10px] text-gray-500 uppercase tracking-widest font-bold mb-6">Zaman Aralığı: {startDate} 14:10 - {endDate} 14:30</p>
            ...
            <div className="mt-4 flex flex-wrap gap-4 justify-center">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">%100 Eşik Çizgisi</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-0.5 h-4 bg-red-500"></div>
                <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">Alarm Anı (14:25:50)</span>
              </div>
            </div>
          </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
          ...
          {/* Cascade Diagram */}
          <motion.div variants={itemVariants} className="bg-white dark:bg-dark-surface p-6 border border-gray-200 dark:border-dark-border rounded-none shadow-sm flex flex-col justify-center">
            <h3 className="text-sm font-bold text-gray-800 dark:text-gray-200 uppercase mb-6 flex items-center gap-2">
              <Activity className="w-4 h-4 text-gray-500" /> Kaskad Hata Zinciri Diyagramı
            </h3>

            <div className="flex flex-col md:flex-row items-center justify-between gap-4 w-full px-4">
              <div className="bg-gray-100 dark:bg-gray-800 border-2 border-gray-400 p-3 text-center flex-1 w-full md:w-auto relative group">
                <span className="text-xs font-bold text-gray-500 uppercase">Aşama 1</span>
                <p className="font-bold text-gray-700 dark:text-gray-200 text-sm mt-1">Takım Aşınması</p>
              ...
              <div className="bg-gray-900 border-2 border-black p-3 text-center flex-1 w-full md:w-auto">
                <span className="text-xs font-bold text-gray-400 uppercase">Sonuç</span>
          ...
          {/* Hypotheses */}
          <motion.div variants={itemVariants} className="bg-white dark:bg-dark-surface p-6 border border-gray-200 dark:border-dark-border rounded-none shadow-sm flex flex-col gap-4">
            <h3 className="text-sm font-bold text-gray-800 dark:text-gray-200 uppercase mb-2 flex items-center gap-2">
              <Search className="w-4 h-4 text-blue-500" /> RCA Hipotez Kartları (AI Tahmini)
            </h3>

            {hypotheses.map(hyp => (
              <div key={hyp.id} className="border border-gray-200 dark:border-dark-border p-3 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-gray-50 dark:bg-dark-bg">
                <div className="flex-1">
                  <h4 className="text-sm font-bold text-gray-900 dark:text-white">{hyp.title}</h4>
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 uppercase">Kanıt: <span className="font-mono">{hyp.proof}</span></p>
                </div>
                <div className="w-full md:w-48">
                  <div className="flex justify-between items-end mb-1">
                    <span className="text-xs font-bold text-gray-500">Güven Puanı</span>
        ...
        {/* BOTTOM PARETO */}
        <motion.div variants={itemVariants} className="bg-white dark:bg-dark-surface p-6 border border-gray-200 dark:border-dark-border rounded-none shadow-sm mt-6">
          <div className="flex justify-between items-center mb-6">
            <div className="flex flex-col gap-1">
              ...
              <span className="text-[10px] text-gray-500 font-mono">Filtrelenen Dönem: {startDate} - {endDate}</span>
            </div>
            <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest border border-gray-200 dark:border-dark-border px-2 py-1">Tıklanabilir</span>
          </div>
        ...
        <motion.div variants={itemVariants} className="mt-6 border-4 border-[#111827] bg-white dark:bg-dark-surface p-6 shadow-2xl relative overflow-hidden">
          ...
          <div className="flex items-center gap-3 mb-6 relative z-10">
            ...
            <div>
              <h3 className="text-lg font-black text-[#111827] dark:text-white uppercase tracking-tighter">Çapraz Makine Etki Analizi</h3>
              <p className="text-xs font-mono text-gray-500 font-bold tracking-widest uppercase">Tesis Geneli (Fleet Impact) Değerlendirmesi | Dönem: {startDate} - {endDate}</p>
            </div>
          </div>
          ...
          <div className="overflow-x-auto pb-4 custom-scrollbar">
            <div className="grid grid-cols-3 gap-6 relative z-10 min-w-[800px] xl:min-w-0">
              {/* Column 1 */}
              <div className="flex flex-col border-2 border-red-500 bg-[#161b22] p-4 rounded-none shadow-[0_0_15px_rgba(239,68,68,0.2)]">
              ...
              {/* Column 2 */}
              <div className="flex flex-col border-2 border-green-500 bg-[#161b22] p-4 rounded-none">
              ...
              {/* Column 3 */}
              <div className="flex flex-col border-2 border-orange-500 bg-[#161b22] p-4 rounded-none shadow-[0_0_15px_rgba(249,115,22,0.2)] relative overflow-hidden">
